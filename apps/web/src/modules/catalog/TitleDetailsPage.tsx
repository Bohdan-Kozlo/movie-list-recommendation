...
export function TitleDetailsPage() {
  ...
  if (titleQuery.isLoading) return <main className="mx-auto w-full max-w-360 px-[4vw] pb-16"><div className="py-40 font-display text-3xl">Loading title…</div></main>
  if (titleQuery.isError || !titleQuery.data) return <main className="mx-auto w-full max-w-360 px-[4vw] pb-16"><button className="border-b border-current pb-0.5 text-[#d9e4eb]" onClick={onBack}>← Back to catalogue</button><CatalogueMessage title="This title is unavailable" body="Return to the catalogue and choose another title." /></main>
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="min-h-[min(54rem,100vh)] bg-[#162636] bg-cover bg-center bg-no-repeat" style={backdrop ? { backgroundImage: `linear-gradient(90deg, #172331 8%, rgba(23,35,49,.84) 42%, rgba(23,35,49,.28)), url(${backdrop})` } : undefined}>
        ...
        <section className="mx-auto grid w-[84vw] max-w-280 grid-cols-[minmax(11rem,19rem)_minmax(0,42rem)] items-end gap-[clamp(2rem,6vw,7rem)] py-[clamp(4rem,12vh,10rem)] pb-20 max-md:grid-cols-1 max-md:pt-16">
          ...
          <div>
            <p className="m-0 font-mono text-xs font-bold uppercase tracking-[.12em] text-[#d4e0e8]">{title.type === 'movie' ? 'Film' : 'TV series'} · {title.releaseDate?.slice(0, 4) ?? '—'}</p>
            <h1 className="mt-2 font-display text-[clamp(2.85rem,5.5vw,5.25rem)] font-semibold leading-[.92] tracking-[-.06em]">{title.title}</h1>
            {title.tagline && <p className="mt-6 font-display text-2xl italic text-primary">{title.tagline}</p>}
            <p className="mt-6 font-mono text-xs uppercase tracking-[.045em] text-[#d4e0e8]">{title.genres.join(' · ')} {title.runtimeMinutes ? `· ${title.runtimeMinutes} min` : ''} {title.voteAverage ? `· ${title.voteAverage.toFixed(1)} / 10` : ''}</p>
            <p className="max-w-152 font-sans text-[1.2rem] leading-[1.52] text-[#e3eaef]">{title.overview || 'No overview is available for this title.'}</p>
            {title.creators.length > 0 && <p className="mt-6 font-mono text-xs uppercase tracking-[.045em] text-[#d4e0e8]"><strong>{title.type === 'movie' ? 'Director' : 'Created by'}</strong> {title.creators.join(', ')}</p>}
