...
export function AccountMenu() {
  ...
  const accountQuery = useQuery({ queryKey: authKeys.currentUser, queryFn: fetchCurrentUser })
  const logoutMutation = useMutation({
    mutationFn: logout,
    onSuccess: () => queryClient.setQueryData(authKeys.currentUser, null),
  })
    ...
    return (
      <details className="group relative">
        <summary className="flex h-10 list-none items-center gap-2 border border-border bg-card px-2.5 font-mono text-xs text-foreground marker:content-none hover:border-primary [&::-webkit-details-marker]:hidden">
          <span
            className="grid size-6 place-items-center rounded-full bg-primary font-bold text-primary-foreground"
            aria-hidden="true"
          >
            {accountQuery.data.email[0]?.toUpperCase()}
          </span>
        ...
        <div className="absolute right-0 z-20 mt-2 grid min-w-56 gap-1 border border-border bg-[#213142] p-2 shadow-2xl">
          ...
          <button
            className="px-3 py-2 text-left text-sm text-muted-foreground hover:bg-[#31485c] hover:text-foreground"
            type="button"
  ...
  return (
    <nav
      className="flex items-center gap-4 font-mono text-xs uppercase tracking-[.06em] text-muted-foreground"
      aria-label="Account"
    >
