...
export function OnboardingPage() {
  ...
  const ratingMutation = useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      queryClient.setQueryData<OnboardingProgress>(['onboarding'], (current) => {
  ...
  const candidates = [...progress.movies, ...progress.tvSeries]
  ...
  if (!title) return <main className="grid min-h-screen place-items-center p-8"><Card className="max-w-xl p-8">No more unrated titles are available right now.</Card></main>
  ...
  return (
    <main className="mx-auto w-full max-w-360 px-[4vw] pb-16">
      ...
      <section className="grid max-w-4xl gap-5 border-b py-[clamp(3rem,7vw,6rem)]">
        ...
        <p className="m-0 font-mono text-xs uppercase tracking-[.08em] text-muted-foreground">{progress.ratingsRemaining} more {progress.ratingsRemaining === 1 ? 'rating' : 'ratings'} to unlock recommendations</p>
      </section>
      <section className="grid gap-8 py-12 lg:grid-cols-[minmax(16rem,22rem)_minmax(0,1fr)] lg:items-center" aria-labelledby="onboarding-title">
        <TitlePoster title={title} />
        <div className="grid max-w-2xl gap-6">
          <p className="m-0 font-mono text-xs font-bold uppercase tracking-[.12em] text-primary">{selectedTitle ? 'Search result' : `Pick ${visibleIndex + 1} of ${candidates.length}`}</p>
          <div><p className="m-0 font-mono text-xs uppercase tracking-[.06em] text-muted-foreground">{title.type === 'movie' ? 'Film' : 'TV series'} · {title.releaseDate?.slice(0, 4) ?? '—'}</p><h2 id="onboarding-title" className="mt-2 font-display text-[clamp(3rem,5vw,5rem)] font-semibold leading-[.92] tracking-[-.06em]">{title.title}</h2><p className="mb-0 mt-4 text-muted-foreground">{title.genres.join(' · ')}</p></div>
          <div className="border-y py-6"><p className="m-0 font-mono text-xs font-bold uppercase tracking-[.12em] text-primary">How much did it land?</p><div className="mt-4"><StarRating label={`Your rating for ${title.title}`} value={rating} onChange={setRating} disabled={ratingMutation.isPending} /></div></div>
          <div className="flex flex-wrap gap-3"><Button type="button" onClick={() => ratingMutation.mutate({ titleId: title.id, value: rating })} disabled={ratingMutation.isPending}>Rate {rating.toFixed(1)} and continue</Button><Button variant="outline" type="button" onClick={previous} disabled={ratingMutation.isPending || (!selectedTitle && visibleIndex === 0)}>← Back</Button><Button variant="ghost" type="button" onClick={next} disabled={ratingMutation.isPending || (!selectedTitle && visibleIndex === candidates.length - 1)}>Next →</Button></div>
      ...
      <section className="border-t py-12" aria-labelledby="onboarding-search-title">
        ...
        <h2 id="onboarding-search-title" className="mt-2 font-display text-4xl font-semibold tracking-[-.05em]">Choose a different title.</h2>
...
function TitlePoster({ title }: { title: TitleCard }) {
  ...
  return <div className="aspect-2/3 overflow-hidden bg-card">{poster ? <img className="h-full w-full object-cover" src={poster} alt="" /> : <span className="grid h-full place-items-center text-sm text-muted-foreground">No image available</span>}</div>
}

function OnboardingError({ message }: { message: string }) {
  return <main className="grid min-h-screen place-items-center p-8"><Card className="max-w-xl p-8"><p role="alert">{message}</p><Button asChild><Link to="/auth/sign-in">Sign in</Link></Button></Card></main>
}
...
function SearchChoice({ titles, onChoose }: { titles: TitleCard[]; onChoose: (title: TitleCard) => void }) {
  ...
  return <div className="mt-6 flex flex-wrap gap-3"><select className="min-w-64 border bg-card px-3.5 py-3" value={titleId} onChange={(event) => setTitleId(event.target.value)} aria-label="Search results">{titles.map((title) => <option key={title.id} value={title.id}>{title.title} · {title.type === 'movie' ? 'Film' : 'TV series'}</option>)}</select><Button type="button" onClick={() => selected && onChoose(selected)}>Rate this title</Button></div>
}
